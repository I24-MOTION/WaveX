...
class LightningDiffusionModel(L.LightningModule):
    def __init__(self, lr=2e-4, time_steps=100):
        super().__init__()
        self.time_steps = time_steps
        self.lr = lr
    ...
    def generate_matrix(self, input_image, time_steps=1000):
        self.eval()
        with torch.no_grad():
            y = torch.randn((1, 1, 200, 200), device=self.device)
            for t in range(time_steps - 1, 0, -1):
                ...
                pred_noise = self.diffusion_model(torch.cat([input_image, y], dim=1), alpha_t_hat.view(-1).to(self.device))
                y = (torch.sqrt(1 / alpha_t)) * (y - (1 - alpha_t) / torch.sqrt(1 - alpha_t_hat) * pred_noise)
                if t > 1:
                    noise = torch.randn_like(y)
                    y = y + torch.sqrt(beta_t) * noise
        y_gen = y.squeeze(0).cpu().numpy()
        y_gen = np.clip(y_gen,0,1)
        return y_gen

    def generate_procedure(self, input_image, time_steps=500):
        self.eval()
        procedure = []
        with torch.no_grad():
            y = torch.randn((1, 1, 200, 200), device=self.device)
            procedure.append(y.squeeze(0).cpu().numpy())  # Save initial noisy image
            for t in range(time_steps - 1, 0, -1):
                alpha_t = self.diffusion_model.alphas[t]
                alpha_t_hat = self.diffusion_model.alpha_hats[t]
                beta_t = self.diffusion_model.betas[t]
                ...
                if t > 1:
                    noise = torch.randn_like(y)
                    y = y + torch.sqrt(beta_t) * noise
                procedure.append(y.squeeze(0).cpu().numpy())  # Save the image at each timestep
        return np.array(procedure)

# visualize the data
import matplotlib.pyplot as plt
import ASMx as ASM
import pandas as pd
def get_speed(speed_matrix: pd.DataFrame, delta: float, tau: float,
              c_cong: float, c_free: float, vthr: float, vdelta: float,
              smoothing_time_window: int = 400, smoothing_space_window: int = 1,
              plot: bool = False) -> np.ndarray:
    """
    Computes the smoothed speed field:
      - Generates weighting matrices using ASM.generate_weight_matrices.
      - Computes the smoothed speed field using ASM.smooth_speed_field.
      - Optionally plots the result.
    """
    cong_weight_matrix, free_weight_matrix = ASM.generate_weight_matrices(
        smoothing_time_window=smoothing_time_window,
        smoothing_space_window=smoothing_space_window,
        delta=delta,
        tau=tau,
        plot=False
    )
    asm_speed = ASM.smooth_speed_field(speed_matrix, cong_weight_matrix, free_weight_matrix,
                                       vthr=vthr, vdelta=vdelta)
    if plot:
        plt.figure(figsize=(12, 6))
        plt.imshow(asm_speed.T, cmap='hot', aspect='auto', vmin=0, vmax=80)
        plt.colorbar(label='Speed')
        plt.title("Smoothed Speed Field")
...
if __name__ == '__main__':
    ...
    model = LightningDiffusionModel.load_from_checkpoint("experiment/lightning_logs/asmx/version_3/checkpoints/best.ckpt")
    # model = LightningDiffusionModel.load_from_checkpoint("experiment/lightning_logs/r2/version_15/checkpoints/best.ckpt")

    # Prepare the datasets
    datasets = {
        "val": WavexDataset("eval/val/rds.npy", "eval/val/motion.npy"),
        "test": WavexDataset("eval/test/rds.npy", "eval/test/motion.npy"),
        "train": WavexDataset("eval/train/rds.npy", "eval/train/motion.npy")
    }
    ...
    for split, dataset in datasets.items():
        ...
        # Create a CSV file to save timing information
        with open(f"results/{split}/timings.csv", mode="w", newline="") as csvfile:
            csv_writer = csv.writer(csvfile)
            csv_writer.writerow(["Index", "Elapsed Time (seconds)"])  # Write the header

            # Iterate over the dataset
            for idx, (input_image, ground_truth) in enumerate(loader):
                ...
                # Perform inference
                model.eval()
                generated_matrix = model.generate_matrix(input_image, time_steps=500)
                results = get_speed(100 * generated_matrix[0].T,
                                    delta=0.02, tau=4, c_cong=12.5, c_free=-45, vdelta=40, vthr=10,
                                    smoothing_space_window=1, smoothing_time_window=200)
                generated_matrix = results.T / 100

                # make it from (200, 200) to (1, 200, 200)
                generated_matrix = np.expand_dims(generated_matrix, axis=0)
                end_time = time.time()
                elapsed_time = end_time - start_time

                # Extract the input's first channel
                input_first_channel = input_image.squeeze(0).cpu().numpy()[0]

                # Convert ground_truth to numpy array
                ground_truth_numpy = ground_truth.squeeze(0).cpu().numpy()

                # Save the generated matrix, input's first channel, ground_truth, and timing in one file
                np.savez(f"results/{split}/ai_{idx}.npz", generated=generated_matrix, input=input_first_channel, ground_truth=ground_truth_numpy, timing=elapsed_time)

                # Write the timing information to the CSV file
                csv_writer.writerow([idx, elapsed_time])

                # Optionally, print a message
                print(f"Saved {split}/ai_{idx}.npz with timing: {elapsed_time:.2f} seconds")

    print("Inference completed for all splits, results saved, and timings recorded.")